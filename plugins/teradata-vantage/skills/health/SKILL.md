...
- `dba_userDelay(start_date, end_date)`: ONE call; rows are queries with `DelayTime > 0` from `DBC.DBQLogTbl` (needs DBQL enabled). Any workload with repeated delay is a TASM/throttle finding. Hand a repeated-delay finding to the `workload` skill: it reads `DBC.TDWMSummaryLog`, which is populated by workload management rather than DBQL and therefore still answers when DBQL is off.
